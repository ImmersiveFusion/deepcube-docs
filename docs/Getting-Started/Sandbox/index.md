...
Explore IAPM with demo data, no instrumentation and no account required. Both sandbox experiences below are **demo grids**: they carry sample telemetry so you can move through a real system in IAPM without sending any data of your own. You view a demo grid in the IAPM client, and we recommend **IAPM**, where a grid renders as a space you move through.
...
| --- | --- | --- |
| **What is it?** | Built-in demo grid inside IAPM | A demo grid you drive from the open-source web app at [demo.iapm.app](https://demo.iapm.app){ target="_blank" } |
...
| **View it in** | IAPM | IAPM or IAPM Web |
...
The Demo grid is a built-in demo grid with a realistic service topology and telemetry, so you can explore the full 3D experience immediately. It appears in the grid picker in IAPM.
...
4. Open the grid in [IAPM](../../DC/3D/index.md) to watch the telemetry appear live (or in [IAPM Web](../../DC/Web/index.md) from the browser)
